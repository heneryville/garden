...
define(['underscore'], function(_){
    console.log('Tropism loaded');
    var graft;
    //TODO: We need a real bus
    var bus = {
        on: function(){},
        emitt: function(){}
    }

    function setGraft(val) {
        graft = val;
    }

    function acquire(behaviorName, el, model, callback) {
        try{
            require([behaviorName],function(behaviorDef){
                if(!behaviorDef) {
                    callback('Could not load behavior: ' + behaviorName);
                    return;
                }
                //Right now we're re-subbing the behavior every time. We can probably do better than that.
                continuation(Behavior.sub(behaviorDef));
            });
        }
        catch(err){
            callback(err);
        ...
        function continuation(behaviorClass) {
